...
class ImageProcessor {
    ...
    getSafeName(p: string, i: number, trip: string) {
        return `${trip.replace(/[^a-z0-9]/gi, '_')}_${i}_clean_${
            basename(p).replace(/[^a-z0-9]/gi, '_').toLowerCase()
        }.jpg`;
    }
    setTrip(name: string) {
    ...
    async process(path: string, index: number, tripName: string, exif: ExifData, gen: number, geo: GeoProxy) {
...
class PhotoCastSystem {
    ...
    public async selectTrip(name?: string, query?: string, restored?: RestoredState) {
        ...
        const trip = trips.find((t: TripItem) => t.name === name) || trips[Math.floor(Math.random() * trips.length)];
        if (!trip) {
            this.isScanning = false;
            return;
        }
        ...
        for (let i = 0; i < files.length; i += 50) {
            ...
            batch.forEach((p) => {
                ...
                metadata.push({
                    path: p,
                    TS: (exif['Date/Time Original'] || exif['Create Date'] || '').toString(),
                    exif,
                });
            });
        }
